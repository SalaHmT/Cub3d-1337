...
/*   Updated: 2024/05/26 01:43:57 by zbendahh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	horizontal_while(t_data *data)
{
			...
			data->cast.next_horz_touch_y <= data->mlx.y_win)
	{
		...
		if (data->map.map[(int)data->cast.y_to_check / TILE_SIZE][(int)data->cast.x_to_check / TILE_SIZE] != 48 && data->map.map[(int)data->cast.y_to_check / TILE_SIZE][(int)data->cast.x_to_check / TILE_SIZE] != 83)	
		{
			...
			data->cast.horz_wall_content = data->map.map[(int)floor(data->cast.y_to_check \
...
void	is_horizontal(t_data *data, float ray_angle)
{
	...
	data->cast.x_intercept = data->player.x + (data->cast.y_intercept - data->player.y) \
		/ tan(ray_angle);
